...
setwd('~/Diffusion_GeoSSE/geosse_all/')
...
e_A <- 0.002 # Extinction rate in region A
e_B <- 0.003 # Extinction rate in region B
e_C <- 0.001 # Extinction rate in region C

e_vec <- c(e_A,e_B,e_C)

# Region-specific dispersal rates 
d_AB <- 0.006 # Dispersal rate from region A to region B
d_BA <- 0.006 # Dispersal rate from region B to region A
d_AC <- 0.003 # Dispersal rate from region A to region C
d_CA <- 0.003 # Dispersal rate from region C to region A
d_BC <- 0.001 # Dispersal rate from region B to region C
d_CB <- 0.001 # Dispersal rate from region C to region B
...
w_A <- 0.09 # Within-region speciation rate in region A
w_B <- 0.06 # Within-region speciation rate in region B
w_C <- 0.07 # Within-region speciation rate in region C

w_vec <- c(w_A,w_B,w_C)

# Between-region speciation rates
b_A_B <- 0.02   # Between-region speciation rate from range {A,B} to {A} and {B}
b_A_C <- 0.02   # Between-region speciation rate from range {A,C} to {A} and {C}
b_B_C <- 0.02   # Between-region speciation rate from range {B,C} to {B} and {C}
b_A_BC <- 0.02  # Between-region speciation rate from range {A,B,C} to {A} and {B,C}
b_B_AC <- 0.02  # Between-region speciation rate from range {A,B,C} to {B} and {A,C}
b_C_AB <- 0.02  # Between-region speciation rate from range {A,B,C} to {C} and {A,B}
...
# only do this for tests other than test for within-speciation only (for now)
