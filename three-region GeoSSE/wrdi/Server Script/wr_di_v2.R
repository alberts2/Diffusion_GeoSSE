# This code simulates range state dynamic for a three-region GeoSSE model with only within-region speciation and dispersal events
...
e_A <- 0 # Extinction rate in region A
e_B <- 0 # Extinction rate in region B
e_C <- 0 # Extinction rate in region C

e_vec <- c(e_A,e_B,e_C)

# Region-specific dispersal rates
d_AB <- 0.03 # Dispersal rate from region A to region B
d_BA <- 0.03 # Dispersal rate from region B to region A
d_AC <- 0.04 # Dispersal rate from region A to region C
d_CA <- 0.04 # Dispersal rate from region C to region A
d_BC <- 0.05 # Dispersal rate from region B to region C
d_CB <- 0.05 # Dispersal rate from region C to region B
...
w_A <- 0.03 # Within-region speciation rate in region A
w_B <- 0.03 # Within-region speciation rate in region B
w_C <- 0.03 # Within-region speciation rate in region C

w_vec <- c(w_A,w_B,w_C)

# Between-region speciation rates
b_A_B <- 0   # Between-region speciation rate from range {A,B} to {A} and {B}
b_A_C <- 0   # Between-region speciation rate from range {A,C} to {A} and {C}
b_B_C <- 0   # Between-region speciation rate from range {B,C} to {B} and {C}
b_A_BC <- 0  # Between-region speciation rate from range {A,B,C} to {A} and {B,C}
b_B_AC <- 0  # Between-region speciation rate from range {A,B,C} to {B} and {A,C}
b_C_AB <- 0  # Between-region speciation rate from range {A,B,C} to {C} and {A,B}
...
N_combined_t <- function(sim,max_T,time_win,r_norm,delta_t){
  ...
  for (j in 2:t){
    ...
    N_C <- N_combined[[j-1]]$N_range[3] + mu_i(3,1,j-1,combined_list,delta_t)*delta_t + r_norm[sim,j-1]*sqrt(var_i(3,1,j-1,combined_list,delta_t)*delta_t)
    N_AB <- N_combined[[j-1]]$N_range[4] + mu_i(4,1,j-1,combined_list,delta_t)*delta_t + r_norm[sim,j-1]*sqrt(var_i(4,1,j-1,combined_list,delta_t)*delta_t)
    N_AC <- N_combined[[j-1]]$N_range[5] + mu_i(5,1,j-1,combined_list,delta_t)*delta_t + r_norm[sim,j-1]*sqrt(var_i(5,1,j-1,combined_list,delta_t)*delta_t)
    N_BC <- N_combined[[j-1]]$N_range[6] + mu_i(6,1,j-1,combined_list,delta_t)*delta_t + r_norm[sim,j-1]*sqrt(var_i(6,1,j-1,combined_list,delta_t)*delta_t)
    N_ABC <- N_combined[[j-1]]$N_range[7] + mu_i(7,1,j-1,combined_list,delta_t)*delta_t + r_norm[sim,j-1]*sqrt(var_i(7,1,j-1,combined_list,delta_t)*delta_t)
...
pC_t_func <- function(sim,time_win,max_T,r_norm,N_sim_t,N_total_t){
  max_T <- max_T
  t <- time_win #time windows 
  time_span <- seq(0,max_T,length.out=t)
  delta_t <- time_span[[2]]-time_span[[1]]
  ...
  N_total_t <- N_total_t #load N_t_total_list
  for (i in 2:t){
    pC_next <- pC_theor_vec[i-1] + mu_Pi_i(3,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + r_norm[sim,i-1]*sqrt(var_Pi_i(3,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
    if (pC_next < 0  || pC_next > 1){
      while (pC_next < 0 || pC_next > 1){
        pC_next<- pC_theor_vec[i-1] + mu_Pi_i(3,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + rnorm(1,0,1)*sqrt(var_Pi_i(3,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
      }
    }
    pC_theor_vec <- append(pC_theor_vec,pC_next)
  }
  return(pC_theor_vec)
}
...
pAB_t_func <- function(sim,time_win,max_T,r_norm,N_sim_t,N_total_t){
  ...
  N_total_t <- N_total_t #load N_t_total_list
  for (i in 2:t){
    pAB_next <- pAB_theor_vec[i-1] + mu_Pi_i(4,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + r_norm[sim,i-1]*sqrt(var_Pi_i(4,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
    if (pAB_next < 0  || pAB_next > 1){
      while (pAB_next < 0 || pAB_next > 1){
        pAB_next<- pAB_theor_vec[i-1] + mu_Pi_i(4,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + rnorm(1,0,1)*sqrt(var_Pi_i(4,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
      }
    }
...
pAC_t_func <- function(sim,time_win,max_T,r_norm,N_sim_t,N_total_t){
  max_T <- max_T
  t <- time_win #time windows 
  time_span <- seq(0,max_T,length.out=t)
  delta_t <- time_span[[2]]-time_span[[1]]
  ...
  N_total_t <- N_total_t #load N_t_total_list
  for (i in 2:t){
    pAC_next <- pAC_theor_vec[i-1] + mu_Pi_i(5,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + r_norm[sim,i-1]*sqrt(var_Pi_i(5,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
    if (pAC_next < 0  || pAC_next > 1){
      while (pAC_next < 0 || pAC_next > 1){
        pAC_next<- pAC_theor_vec[i-1] + mu_Pi_i(5,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) +rnorm(1,0,1)*sqrt(var_Pi_i(5,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
      }
    }
    pAC_theor_vec <- append(pAC_theor_vec,pAC_next)
  }
  return(pAC_theor_vec)
}

pBC_t_func <- function(sim,time_win,max_T,r_norm,N_sim_t,N_total_t){
  max_T <- max_T
  t <- time_win #time windows 
  time_span <- seq(0,max_T,length.out=t)
  delta_t <- time_span[[2]]-time_span[[1]]
  ...
  N_total_t <- N_total_t #load N_t_total_list
  for (i in 2:t){
    pBC_next <- pBC_theor_vec[i-1] + mu_Pi_i(6,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + r_norm[sim,i-1]*sqrt(var_Pi_i(6,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
    if (pBC_next < 0  || pBC_next > 1){
      while (pBC_next < 0 || pBC_next > 1){
        pBC_next<- pBC_theor_vec[i-1] + mu_Pi_i(6,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + rnorm(1,0,1)*sqrt(var_Pi_i(6,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
      }
    }
    pBC_theor_vec <- append(pBC_theor_vec,pBC_next)
  }
  return(pBC_theor_vec)
}

pABC_t_func <- function(sim,time_win,max_T,r_norm,N_sim_t,N_total_t){
  max_T <- max_T
  t <- time_win #time windows 
  time_span <- seq(0,max_T,length.out=t)
  delta_t <- time_span[[2]]-time_span[[1]]
  ...
  N_total_t <- N_total_t #load N_t_total_list
  for (i in 2:t){
    pABC_next <- pABC_theor_vec[i-1] + mu_Pi_i(7,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + r_norm[sim,i-1]*sqrt(var_Pi_i(7,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
    if (pABC_next < 0  || pABC_next > 1){
      while (pABC_next < 0 || pABC_next > 1){
        pABC_next<- pABC_theor_vec[i-1] + mu_Pi_i(7,sim,i-1,N_total_t,N_sim_t,delta_t)*(delta_t) + rnorm(1,0,1)*sqrt(var_Pi_i(7,sim,i-1,N_total_t,N_sim_t,delta_t)*delta_t)
      }
    }
    pABC_theor_vec <- append(pABC_theor_vec,pABC_next)
  }
  return(pABC_theor_vec)
}
...
pC_theor_list_func <- function(n_sim,time_win,max_T,r_norm,N_sim_t,N_total_t,preschedule,n_cores){
  pC_theor_list <- mclapply(c(1:n_sim), pC_t_func,time_win=time_win,max_T=max_T,r_norm=r_norm,N_sim_t=N_sim_t,N_total_t=N_total_t,mc.preschedule = preschedule,mc.cores = n_cores)
  return(pC_theor_list)
}
...
pAC_theor_list_func <- function(n_sim,time_win,max_T,r_norm,N_sim_t,N_total_t,preschedule,n_cores){
  pAC_theor_list <- mclapply(c(1:n_sim), pAC_t_func,time_win=time_win,max_T=max_T,r_norm=r_norm,N_sim_t=N_sim_t,N_total_t=N_total_t,mc.preschedule = preschedule,mc.cores = n_cores)
  return(pAC_theor_list)
}

pBC_theor_list_func <- function(n_sim,time_win,max_T,r_norm,N_sim_t,N_total_t,preschedule,n_cores){
  pBC_theor_list <- mclapply(c(1:n_sim), pBC_t_func,time_win=time_win,max_T=max_T,r_norm=r_norm,N_sim_t=N_sim_t,N_total_t=N_total_t,mc.preschedule = preschedule,mc.cores = n_cores)
  return(pBC_theor_list)
}

pABC_theor_list_func <- function(n_sim,time_win,max_T,r_norm,N_sim_t,N_total_t,preschedule,n_cores){
  pABC_theor_list <- mclapply(c(1:n_sim), pABC_t_func,time_win=time_win,max_T=max_T,r_norm=r_norm,N_sim_t=N_sim_t,N_total_t=N_total_t,mc.preschedule = preschedule,mc.cores = n_cores)
  return(pABC_theor_list)
}
...
r_norm_vec = r_norm_vec_func(num_sim = 100,time_win = 1000)
# Generate N_i(t) for each num_sim replicate over time
N_combined_list <- N_t_func(num_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec,TRUE,2)
# Generate N(t) for each num_sim replicate over time
N_t_total_list <- N_t_total_func(num_sim = 100,time_win = 1000, N_sim_t = N_combined_list)
# Generate the num_sim replicates of theoretical frequency for each range state over time
pA_theor_list <- pA_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pB_theor_list <- pB_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pC_theor_list <- pC_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pAB_theor_list <- pAB_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pAC_theor_list <- pAC_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pBC_theor_list <- pBC_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)
pABC_theor_list <- pABC_theor_list_func(n_sim = 100, time_win = 1000, max_T = 10,r_norm = r_norm_vec, N_sim_t = N_combined_list, N_total_t =N_t_total_list, preschedule = TRUE,n_cores = 2)

# Normalized each frequency trajectory for each range state 
pA_theor_list_norm <- vector("list",100)
pB_theor_list_norm <- vector("list",100)
pC_theor_list_norm <- vector("list",100)
pAB_theor_list_norm <- vector("list",100)
pAC_theor_list_norm <- vector("list",100)
pBC_theor_list_norm <- vector("list",100)
pABC_theor_list_norm <- vector("list",100)

for (i in 1:100){
  for (j in 1:1000){
    pA_theor_list_norm[[i]][j] <- pA_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pB_theor_list_norm[[i]][j] <- pB_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pC_theor_list_norm[[i]][j] <- pC_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pAB_theor_list_norm[[i]][j] <- pAB_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pAC_theor_list_norm[[i]][j] <- pAC_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pBC_theor_list_norm[[i]][j] <- pBC_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
    pABC_theor_list_norm[[i]][j] <- pABC_theor_list[[i]][j]/(pA_theor_list[[i]][j]+pB_theor_list[[i]][j]+pC_theor_list[[i]][j]+pAB_theor_list[[i]][j]+pAC_theor_list[[i]][j]+pBC_theor_list[[i]][j]+pABC_theor_list[[i]][j])
  }
}
